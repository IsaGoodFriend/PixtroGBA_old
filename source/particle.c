...
void AddParticle(int att1, int att2, int att3) {
	...
	memcpy(&tile_mem[4][lastParticle / 3], &particles[PARTICLE_FRAME(lastParticle)], SPRITE_8x8);
	...
	for (index = 0; index < PARTICLE_MAX; index += PARTICLE_DATA_SIZE) {
		...
		if (!(particle_data[index] & ATT1_LIFE)){
			if (particle_data[index + 1] & ATT2_ANIMFRAME){
				...
				memcpy(&tile_mem[4][index / 3], &particles[PARTICLE_FRAME(index)], SPRITE_8x8);
			} 
			else
				continue;
		}
		...
		ATTR2_PALBANK((particle_data[index + 1] & ATT2_PAL) >> ATT2_PAL_S) | (index / 3) | ATTR2_PRIO((particle_data[index + 1] & ATT2_PRIO) >> ATT2_PRIO_S));		// ATTR2

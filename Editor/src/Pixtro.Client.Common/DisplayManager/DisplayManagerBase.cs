...
using Pixtro.Emuware.BizwareGL.DrawingExtensions;
using Pixtro.Client.Common.FilterManager;
using Pixtro.Client.Common.Filters;
...
namespace Pixtro.Client.Common
{
	...
	public class DisplayManagerBase : IDisposable
	{
			...
			IGuiRenderer renderer)
		{
			...
			{
				using var xml = Pixtro.Common.ReflectionCache.EmbeddedResourceStream("Resources.courier16px.fnt");
				using var tex = Pixtro.Common.ReflectionCache.EmbeddedResourceStream("Resources.courier16px_0.png");
				_theOneFont = new StringRenderer(_gl, xml, tex);
				using var gens = Pixtro.Common.ReflectionCache.EmbeddedResourceStream("Resources.gens.ttf");
				LoadCustomFont(gens);
				using var fceux = Pixtro.Common.ReflectionCache.EmbeddedResourceStream("Resources.fceux.ttf");

...
namespace Pixtro.Client.Editor
{
	public class Win32LuaLibraries : IPlatformLuaLibEnv
	{
			...
			IGameInfo game)
		{
			...
			Docs.Clear();
			_apiContainer = ApiManager.RestartLua(serviceProvider, LogToLuaConsole, _mainForm, _displayManager, _inputManager, _mainForm.Tools, config, emulator, game);

			// Register lua libraries
			foreach (var lib in Pixtro.Common.ReflectionCache.Types.Concat(Pixtro.Common.ReflectionCache.Types)
		...
		public void Restart(
			...
			IGameInfo game)
		{
			_apiContainer = ApiManager.RestartLua(newServiceProvider, LogToLuaConsole, _mainForm, _displayManager, _inputManager, _mainForm.Tools, config, emulator, game);

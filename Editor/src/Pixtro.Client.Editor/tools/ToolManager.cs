...
using Pixtro.Common;
using Pixtro.Common.ReflectionExtensions;
...
namespace Pixtro.Client.Editor
{
	public class ToolManager
	{
		...
		public IEnumerable<Type> AvailableTools => Pixtro.Common.ReflectionCache.Types
		...
		private IToolForm CreateInstance(Type toolType, string dllPath, string toolTypeName = null, bool skipExtToolWarning = false)
		{
			...
			if (toolType == typeof(IExternalToolForm))
			{
				...
				try
				{
					tool = Activator.CreateInstanceFrom(dllPath, toolTypeName ?? "Pixtro.Client.Editor.CustomMainForm").Unwrap() as IExternalToolForm;
					if (tool == null)
					{

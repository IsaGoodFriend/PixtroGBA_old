...
using Pixtro.Common.IOExtensions;

namespace Pixtro.Emulation.Cores.Properties {
	internal static class Resources {
		/// <param name="embedPath">Dir separator is '<c>.</c>'. Path is relative to <c>&lt;NS></c>.</param>
		private static byte[] ReadEmbeddedByteArray(string embedPath) => Pixtro.Common.ReflectionCache.EmbeddedResourceStream($"Resources.{embedPath}").ReadAllBytes();

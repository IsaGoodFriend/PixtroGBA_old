...
namespace Pixtro.Emulation.Cores.Waterbox
{
	public abstract unsafe partial class NymaCore : WaterboxCore
	{
		...
		protected T DoInit<T>(GameInfo game, byte[] rom, string wbxFilename, string extension, bool deterministic,
			IDictionary<string, FirmwareID> firmwares = null)
			where T : LibNymaCore
		{
			...
			}, new Delegate[] { _settingsQueryDelegate, firmwareDelegate });
			_nyma = t;
			...
			using (_exe.EnterExit())
			{
				...
				InitControls(portData, false, ref info);
				PostInit();
